...
> | **can1** | **右腿**（ID 1-5）|
>
> **怎么测的**：机器人吊着、电机全失能，用手只动一条腿、另一条不动，
> 差分每条总线的读数（`scripts/sample_motors.py`）：
> ```
> 动右腿 → can1 变化 64.6°，can0 变化 0.00°
> 动左腿 → can0 变化 42.8°，can1 变化 0.00°
> ```
> 两个方向都测，都干净。
>
> ⚠️ **上面写的是【改名后】的名字。** 测的那天接口还叫 `can2`（右）/`can1`（左）——
> 当时板载 CAN 先抢占了 `can0`，USB 适配器只能拿到 can1/can2。后来禁掉板载 CAN
> （`systemctl disable can4.service` + 注释 `98-onboard-can.rules`）之后，
> 同样的两个物理通道才拿到 `can0`/`can1`。
> **接线一根没动，只是名字整体降了一位。**
>
> ⚠️ **2026-09-08 曾得出相反结论**（"can2=左腿、can1=右腿，和直觉相反"），
> 那个结论是**错的**，根因未查清（可能当时接线不同，或判断依据有误）。
> ⚠️ 另有两处外部文档仍写旧结论：
> `reports/达妙下肢_电机ID规划_权威版.md`（已加更正）、`reports/RDK机器人后续路线图.md`。
